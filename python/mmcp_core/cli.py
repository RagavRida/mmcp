...
def cmd_auto(args: argparse.Namespace) -> None:
    ...
    from .skill_engine import find_similar_skill, load_skill, save_skill, list_skills
    ...
    if similar:
        ...
        if use_skill not in ("n", "no"):
            ...
            if plan:
                plan.task = task  # update task but reuse steps
                print(f"\n{GREEN}Using saved skill with {len(plan.steps)} steps{RESET}")
            else:
                similar = None  # fallback to planning
    ...
    # Show final output (last completed step)
    final_output = None
...
def cmd_skills(_args: argparse.Namespace) -> None:
    ...
    from .skill_engine import list_skills, delete_skill
